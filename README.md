...
### View Block Contents: `viewblock`

Displays full details of a block.

Format: `viewblock INDEX`

Constraints:
* `INDEX` must be a non-negative integer (e.g., 0, 1, 2, ...).
* `INDEX` must be smaller than the current blockchain size.
...
Previous Hash : 0000ab82f91d2...
Current Hash  : 0000f9812a773...
Transactions:
alice -> bob : 10
bob -> carol : 5
```
...
* Recomputes hash of each block
* Verifies stored hash matches computed hash
...
```
Blockchain is invalid. Reason: Hash mismatch at Block 3.
```

### Testing Funding: `faucet`

Funds a wallet for local testing so transactions can be executed.

Format: `faucet w/WALLET_NAME amt/AMOUNT`
...
> faucet w/alice amt/10
Funded wallet alice with 10
